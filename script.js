...
(function(){
  ...
  // ── Cached, resize-only state ──
  // groundY / carX are constant for a given canvas size (the car does not
  // move horizontally frame-to-frame — only the background scrolls and the
  // wheels spin), so they are computed once per resize instead of every frame.
  var groundY = 0, carX = 0;

  // Gradients are expensive to construct and every one of these is fully
  // determined by W/H/carX/groundY, none of which change between frames.
  // Built once here, rebuilt only when rebuildCache() runs (init + resize).
  var skyGrad, horizonGrad, roadGrad, reflectionGrad;

  // Cached per-frame-constant car geometry + the 3 car-local gradients.
  var carGeo = null;
  ...
  function rebuildCache(){
    ...
    // ── Frame-level gradients ──
    skyGrad = cx.createLinearGradient(0,0,0,H*0.52);
    skyGrad.addColorStop(0,'#06070b');
    skyGrad.addColorStop(1,'#0d0f16');

    horizonGrad = cx.createLinearGradient(0, H*0.38, 0, H*0.52);
    horizonGrad.addColorStop(0,'rgba(201,168,76,0.0)');
    horizonGrad.addColorStop(1,'rgba(201,168,76,0.06)');

    roadGrad = cx.createLinearGradient(0, groundY, 0, H);
    roadGrad.addColorStop(0,'#131418');
    roadGrad.addColorStop(1,'#0d0e11');

    reflectionGrad = cx.createLinearGradient(0, groundY, 0, H);
    reflectionGrad.addColorStop(0,'rgba(201,168,76,0.04)');
    reflectionGrad.addColorStop(1,'transparent');

    // ── Car geometry (identical formulas to the original drawCar()) ──
    var cw = W * 0.40;
    var wr  = cw * 0.118;
    var wxF = carX + cw * 0.28;
    var wxR = carX - cw * 0.27;
    var wY  = groundY - wr;

    var bodyL  = carX - cw * 0.46;
    var bodyR  = carX + cw * 0.46;
    var bodyBY = wY + wr * 0.45;
    var bodyTY = wY - wr * 2.1;
    var bodyH  = bodyBY - bodyTY;

    var cabL  = carX - cw * 0.25;
    var cabR  = carX + cw * 0.30;
    var cabTY = bodyTY - bodyH * 0.52;
    var cabBY = bodyTY;
    ...
    // ── Car-local gradients (positions derive only from the geometry above) ──
    ...
    var shadowGrad = cx.createRadialGradient(carX, groundY + 2, 0, carX, groundY + 2, cw * 0.42);
    shadowGrad.addColorStop(0, 'rgba(0,0,0,0.5)');
    shadowGrad.addColorStop(1, 'transparent');

    var underglowGrad = cx.createRadialGradient(carX, groundY, 0, carX, groundY, cw * 0.38);
    underglowGrad.addColorStop(0, 'rgba(201,168,76,0.28)');
    underglowGrad.addColorStop(1, 'transparent');

    carGeo = {
      cw:cw, wr:wr, wxF:wxF, wxR:wxR, wY:wY,
      bodyL:bodyL, bodyR:bodyR, bodyBY:bodyBY, bodyTY:bodyTY, bodyH:bodyH,
      cabL:cabL, cabR:cabR, cabTY:cabTY, cabBY:cabBY,
      winMidL:winMidL, winMidR:winMidR, midDiv:midDiv,
      beamGrad:beamGrad, shadowGrad:shadowGrad, underglowGrad:underglowGrad
    };
  }
  ...
  // Arch trim + wheel drawing, hoisted to named functions defined ONCE
  // (previously recreated as closures inside a [wxR,wxF].forEach(...) on
  // every single frame). Called directly, twice, per frame — no array,
  // no forEach, no per-frame function allocation.
  function archTrimDraw(wx, wY, wr) {
    cx.beginPath();
  ...
  function drawCar(carX, groundY) {
    var g = carGeo;
    var cw = g.cw, wr = g.wr, wxF = g.wxF, wxR = g.wxR, wY = g.wY;
    var bodyL = g.bodyL, bodyR = g.bodyR, bodyBY = g.bodyBY, bodyTY = g.bodyTY, bodyH = g.bodyH;
    var cabL = g.cabL, cabR = g.cabR, cabTY = g.cabTY, cabBY = g.cabBY;
    var winMidL = g.winMidL, winMidR = g.winMidR, midDiv = g.midDiv;
    ...
    cx.closePath();
    cx.fillStyle = g.beamGrad;
    cx.fill();

    // ── Ground shadow ──
    cx.fillStyle = g.shadowGrad;
    cx.beginPath();
    cx.ellipse(carX, groundY + 4, cw * 0.4, 9, 0, 0, Math.PI * 2);
    cx.fill();

    // ── Neon underglow ──
    cx.fillStyle = g.underglowGrad;
    cx.beginPath();
    ...
    archTrimDraw(wxR, wY, wr);
    archTrimDraw(wxF, wY, wr);
  ...
  function frame(){
    ...
    // ── Sky ──
    cx.fillStyle = skyGrad; cx.fillRect(0,0,W,H*0.52);
    ...
    cx.fillStyle = horizonGrad; cx.fillRect(0,H*0.38,W,H*0.14);

    // ── Background buildings (slow parallax) ──
    var bgOffset = (t * 0.3) % W;
    for(var rep=0;rep<3;rep++){
      for(var bi=0; bi<bldgsBg.length; bi++){
        var b = bldgsBg[bi];
        var bx = ((b.x * W - bgOffset + rep*W) % (W*1.5)) - W*0.1;
        var bh = b.h * H;
        var bw = b.w * W;
        if (bx + bw < 0 || bx > W) continue; // viewport culling — fully offscreen, skip
        cx.fillStyle = '#0f1018';
    ...
    for(var rep2=0;rep2<3;rep2++){
      for(var fi=0; fi<bldgsFg.length; fi++){
        var b = bldgsFg[fi];
